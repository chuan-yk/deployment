...
from .models import RecordOfjavajar
from cmdb.models import ProjectInfo
from cmdb.mytools import file_as_byte_md5sum
from cmdb.remotepubtomcatjar import RemoteZipReplaceWorker

redis_for_app_cli = get_redis_connection("default")


class IndexView(generic.ListView):
    template_name = 'tomcatjar/tomcat_list.html'
    model = Fileupload
    ...
    def get_queryset(self):
        ...
        return Fileupload.objects.filter(type=3).order_by("-pk", )
...
def list_detail(request, pk):
    ...
    return render(request, 'tomcatjar/tomcat_detail.html', context)
...
def apppub(request, pk):
    ...
    return redirect(reverse('tmct_jar_url_tag:file_detail', args=[pk, ]))
